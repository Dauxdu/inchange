...
export const SkeletonCard = ({ isDark }) => {
  return (
    <div className={`p-6 h-full rounded-3xl border ${
      isDark ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white/90 border-gray-200/50'
    }`}>
      <div className="animate-pulse">
        ...
        <div className="flex gap-3">
          <div className={`h-8 flex-1 rounded-full ${
            isDark ? 'bg-gray-700' : 'bg-gray-200'
          }`} />
          <div className={`h-8 flex-1 rounded-full ${
            isDark ? 'bg-gray-700' : 'bg-gray-200'
    ...
    <div className="relative">
      ...
      {[1, 2, 3].map((i) => (
        <div key={i} className="relative flex items-start mb-12 last:mb-0">
          <div className={`relative z-10 flex-shrink-0 w-16 h-16 rounded-full border-4 ${
            isDark ? 'bg-gray-800 border-gray-600' : 'bg-gray-100 border-gray-300'
          }`} />
          
          <div className={`ml-8 flex-grow p-6 rounded-2xl border ${
            isDark ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white/90 border-gray-200/50'
          }`}>
            <div className="animate-pulse">
              <div className={`h-6 rounded mb-3 w-2/3 ${
                isDark ? 'bg-gray-700' : 'bg-gray-200'
              }`} />
              <div className={`h-4 rounded mb-2 w-1/2 ${
                isDark ? 'bg-gray-700' : 'bg-gray-200'
              }`} />
              <div className={`h-4 rounded mb-2 ${
                isDark ? 'bg-gray-700' : 'bg-gray-200'
...
export const ProgressiveLoader = ({ children, isLoading, skeleton, isDark }) => {
  if (isLoading) {
